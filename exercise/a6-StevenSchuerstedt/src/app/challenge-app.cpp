...
public: 
	virtual double evaluate(const VectorXd &u_stack) const { 
		...
		Vector2d r = ((x.back() - x_sun)).normalized();

		O += 500*(x.back() - r * r_prime).squaredNorm();

		O += 12*pow(r.dot(v.back()), 2);

		O += 373*pow(v.back().norm() - std::sqrt(1 / (m * r_prime) - 0.06), 2);

		for (int i = 0; i < u.size(); i++) {
			O +=  pow(10., -7.6) * u[i].squaredNorm();
		}
