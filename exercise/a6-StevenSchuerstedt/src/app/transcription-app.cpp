...
public:
	...
	virtual double evaluate(const VectorXd &stack) const {
		...
		// --
		double O = 0.; 
		O += 3 * (x.back() - x_prime).squaredNorm();
		O += 3 * (v.back()).squaredNorm();
		O += 3 * (x.front() - x0).squaredNorm();
		O += 3 * (v.front() - v0).squaredNorm();

		for (int i = 0; i < x.size(); i++) {
			O += 0.5 * pow(10., log_c_reg) * u[i].squaredNorm();
			O += 0.5 * pow(10., log_c_reg) * x[i].squaredNorm();
			O += 0.5 * pow(10., log_c_reg) * v[i].squaredNorm();

			if (i < x.size() - 1) {
				O += 10000 * ((x.at(i + 1) - x.at(i)) -  h * v.at(i)).squaredNorm();
				O += 10000 * ((v.at(i + 1) - v.at(i)) - h * u.at(i)/m).squaredNorm();
			}
		}
