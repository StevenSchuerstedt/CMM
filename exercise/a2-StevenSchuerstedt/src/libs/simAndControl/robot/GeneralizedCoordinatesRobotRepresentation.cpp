...
P3D GeneralizedCoordinatesRobotRepresentation::
    ...
    V3D v = V3D(getJointForQIdx(qIndex)->cJPos, pLocal);
    return getJointForQIdx(qIndex)->pJPos +
           rotateVec(v, getQVal(qIndex),
                     getQAxis(qIndex));
 
}


P3D GeneralizedCoordinatesRobotRepresentation::getWorldCoordinatesRecursive(const P3D &p,
                                                       int qIndex) { 

    if (qIndex > 5)
        return getWorldCoordinatesRecursive(
            getCoordsInParentQIdxFrameAfterRotation(qIndex, p),
            getParentQIdxOf(qIndex));
...
P3D GeneralizedCoordinatesRobotRepresentation::getWorldCoordinates(const P3D &p,
    ...
    P3D bla = getWorldCoordinatesRecursive(p, getQIdxForJoint(rb->pJoint));
 
    V3D v_inGrandParent(bla.x, bla.y, bla.z);

    P3D pbase(getQVal(0), getQVal(1), getQVal(2));
    P3D pInWorld = pbase + 
        rotateVec(rotateVec(rotateVec(v_inGrandParent,getQVal(3), V3D(0.0f, 1.0f, 0.0f)), getQVal(4), V3D(1.0f, 0.0f, 0.0f)),getQVal(5), V3D(0.0f, 0.0f, 1.0f));
...
void GeneralizedCoordinatesRobotRepresentation::estimate_linear_jacobian(
    ...
    for (int i = 0; i < q.size(); i++) {
        ...
        V3D dpdq_i = V3D(p_p - p_m);  // TODO: fix this: compute derivative dp(q)/dqi
        dpdq_i /= (2.0 * h);
